...
if(isset($_POST["action"]))
{
	...
	{
		$order_column = array('doctor_table.doctor_name', 'doctor_table.doctor_status');
		...
		ON doctor_table.specs_id = specialization_table.specs_id
		';
		...
		{
			$search_query .= 'WHERE doctor_table.doctor_email_address LIKE "%'.$_POST["search"]["value"].'%" ';
			$search_query .= 'OR doctor_table.doctor_name LIKE "%'.$_POST["search"]["value"].'%" ';
			$search_query .= 'OR doctor_table.doctor_phone_no LIKE "%'.$_POST["search"]["value"].'%" ';
			$search_query .= 'OR doctor_table.doctor_date_of_birth LIKE "%'.$_POST["search"]["value"].'%" ';
			...
			$search_query .= 'OR doctor_table.doctor_prc LIKE "%'.$_POST["search"]["value"].'%" ';
			$search_query .= 'OR doctor_table.doctor_status LIKE "%'.$_POST["search"]["value"].'%" ';
		}
		...
		{
			$order_query = 'ORDER BY doctor_table.doctor_id DESC ';
		}
		...
		else
		{
			...
			if($error == '')
			{
				$data = array(
					...
					':specs_id'						=>	$object->clean_input($_POST["specs_id"]),
					':doctor_prc'					=>	$object->clean_input($_POST["doctor_prc"]),
					':clinic_name'					=>	$object->clean_input($_POST["clinic_name"]),
					':clinic_address'				=>	$object->clean_input($_POST["clinic_address"]),
				...
				(doctor_email_address, doctor_password, doctor_name, doctor_profile_image, doctor_phone_no, doctor_address, doctor_date_of_birth, doctor_degree, specs_id, doctor_prc, clinic_name, clinic_address, doctor_status, doctor_added_on) 
				VALUES (:doctor_email_address, :doctor_password, :doctor_name, :doctor_profile_image, :doctor_phone_no, :doctor_address, :doctor_date_of_birth, :doctor_degree, :specs_id, :doctor_prc, :clinic_name, :clinic_address, :doctor_status, :doctor_added_on)
				";
	...
	if($_POST["action"] == 'fetch_single')
	{
		...
		foreach($result as $row)
		{
			...
			$data['specs_id'] = $row['specs_id'];
			$data['doctor_prc'] = $row['doctor_prc'];
			$data['clinic_name'] = $row['clinic_name'];
			$data['clinic_address'] = $row['clinic_address'];
		}
