...
if(isset($_POST["action"]))
{
	...
	if($_POST['action'] == 'fetch_schedule')
	{
		...
		$order_column = array('doctor_table.doctor_name', 'doctor_table.specs_name', 'doctor_schedule_table.doctor_schedule_date', 'doctor_schedule_table.doctor_schedule_day', 'doctor_schedule_table.doctor_schedule_start_time');
		
		$main_query = "
		SELECT doctor_schedule_table.doctor_schedule_id, doctor_table.doctor_id,
		doctor_table.doctor_name, doctor_table.clinic_name, specialization_table.specs_name, doctor_schedule_table.doctor_schedule_date, 
		doctor_schedule_table.doctor_schedule_day, doctor_schedule_table.doctor_schedule_start_time, doctor_schedule_table.doctor_schedule_end_time
		FROM doctor_schedule_table 
		INNER JOIN doctor_table
		ON doctor_table.doctor_id = doctor_schedule_table.doctor_id
		INNER JOIN specialization_table 
		ON specialization_table.specs_id = doctor_table.specs_id
		...
		AND appointment_table.status IS NULL
    	AND specialization_table.specs_name = "'.$_SESSION['specs'].'"
    	';
		}
		...
		if(isset($_POST["search"]["value"]))
		{
			...
			$search_query .= 'OR doctor_table.clinic_name LIKE "%'.$_POST["search"]["value"].'%" ';
			$search_query .= 'OR specialization_table.specs_name LIKE "%'.$_POST["search"]["value"].'%" ';
		...
		foreach($result as $row)
		{
			$end        = $row["doctor_schedule_end_time"];
			$start      = $row["doctor_schedule_start_time"];
			$endhour    = date("g:i a", strtotime($end));
			$starthour  = date("g:i a", strtotime($start));

			$sub_array = array();

			$sub_array[] = $row["doctor_name"];

			$sub_array[] = $row["clinic_name"];

			$sub_array[] = $row["specs_name"];

			$sub_array[] = $row["doctor_schedule_date"];

			$sub_array[] = $row["doctor_schedule_day"];

			$sub_array[] = ''.$starthour.'-'.$endhour.'';
	...
	if($_POST['action'] == 'book_appointment')
	{
		...
		else
		{
			...
			(doctor_id, patient_id, doctor_schedule_id, reason_for_appointment, status) 
			VALUES (:doctor_id, :patient_id, :doctor_schedule_id, :reason_for_appointment, :status)
			";

			$object->execute($data);

			$_SESSION['appointment_message'] = '<div class="alert alert-success">Your Appointment has been <b>'.$status.'</b></div>';
		}
	...
	if($_POST['action'] == 'fetch_appointment')
	{
		...
		$order_column = array('doctor_table.doctor_name', 'doctor_schedule_table.doctor_schedule_date', 'doctor_schedule_table.doctor_schedule_day', 'appointment_table.status');
		...
		foreach($result as $row)
		{
			...
			if($row["status"] == 'Booked')
			{
				...
				$cancel = '<button type="button" name="cancel_appointment" class="btn btn-danger btn-sm cancel_appointment" data-id="'.$row["appointment_id"].'"><i class="fas fa-times"></i></button>';
			...
			$sub_array[] = '<a href="file_pdf.php?id='.$row["appointment_id"].'" class="btn btn-danger btn-sm" target="_blank"><i class="fas fa-file-pdf"></i> PDF</a>';
