...
if(isset($_POST["doc_email_address"]))
{
	sleep(2);
	$error = '';
	$url = '';
	$data = array(
		':doc_email_address'	=>	$_POST["doc_email_address"]
	);
	...
	if($total_row == 0)
	{
		$object->query = "
			...
			WHERE doctor_email_address = :doc_email_address
		";
		$object->execute($data);
		...
		else
		{
			...
			foreach($result as $row)
			{
				...
				{
					if($_POST["doc_password"] == $row["doctor_password"])
					{
						$_SESSION['doctor_id'] = $row['doctor_id'];
						// $_SESSION['type'] = 'Doctor';
						$url = $object->base_url . 'doctor/dashboard.php';
					}
					else
					{
